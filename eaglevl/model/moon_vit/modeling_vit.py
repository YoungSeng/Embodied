...
class MoonVitEncoder(nn.Module):
    ...
    def forward(
        ...
        detail_layer_indices: Optional[Sequence[int]] = None,
        ...
        selected_states = {}
        if detail_layer_indices is not None:
            selected_indices = tuple(int(index) for index in detail_layer_indices)
            invalid = [index for index in selected_indices if index < 0 or index >= len(self.blocks)]
            if invalid:
                raise ValueError(
                    f"detail_layer_indices contains invalid layers {invalid}; encoder has {len(self.blocks)} layers"
                )
        ...
        for layer_index, block in enumerate(self.blocks):
            ...
            if selected_indices is not None and layer_index in selected_indices:
                selected_states[layer_index] = hidden_states

        hidden_states = self.final_layernorm(hidden_states)

        if selected_indices is not None:
            # The final pyramid level should match the representation consumed
            # by the original patch merger.
            final_index = len(self.blocks) - 1
            if final_index in selected_states:
                selected_states[final_index] = hidden_states
            return hidden_states, tuple(selected_states[index] for index in selected_indices)
...
class MoonVitPretrainedModel(PreTrainedModel):
    ...
    def forward(
        ...
        grid_hws: torch.Tensor,
        output_detail_features: bool = False,
        detail_layer_indices: Optional[Sequence[int]] = None,
    ) -> Union[List[torch.Tensor], Tuple[List[torch.Tensor], Tuple[torch.Tensor, ...]]]:
        """
        Args:
        ...
        if output_detail_features:
            if detail_layer_indices is None:
            ...
            hidden_states, detail_features = self.encoder(
                hidden_states, grid_hws, detail_layer_indices=detail_layer_indices
            )
        else:
            hidden_states = self.encoder(hidden_states, grid_hws)
            detail_features = None
        merged_hidden_states = patch_merger(
            hidden_states, grid_hws, merge_kernel_size=self.merge_kernel_size
        )
        if output_detail_features:
            return merged_hidden_states, detail_features
