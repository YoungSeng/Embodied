...
class LocateAnythingForConditionalGeneration(LocateAnythingPreTrainedModel, GenerationMixin):
    ...
    def extract_ui_features(
        ...
        relation_family,
        defect_type,
        return_global_visual_cache=True,
    ):
        ...
        image_flags = torch.tensor(
            [len(image_grid_hws)], device=relation_family.device, dtype=torch.long
        )
        ...
        return vit_embeds, relation_output, cache
    ...
    def get_last_ui_defect_interface(self):
        """Return relation/PBD tensors from the most recent generation call."""
        return self._last_ui_defect_interface

    def ui_relation_parameter_report(self):
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        if defect_type is not None and not isinstance(defect_type, torch.Tensor):
            defect_type = torch.tensor([defect_type], device=relation_device, dtype=torch.long)
        ...
        if visual_features is not None:
            ...
            vit_embeds, relation_output, global_visual_cache = self.extract_ui_features(
                pixel_values,
                image_grid_hws,
                relation_family.to(pixel_values.device),
                defect_type.to(pixel_values.device),
            )
        ...
        if return_ui_defect_interface:
            return response[0], self._last_ui_defect_interface
        return response[0]
