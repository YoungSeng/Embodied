...
## 2026-09-08 评分目录已存在时的续跑修复

上传日志显示 14 项推理 worker 均已完成，包括独占 GPU 的 synth_loneword；随后外部 UI5 评分器在 `evaluation/raw/ui14-step-0` 执行 `mkdir(exist_ok=False)` 抛出 FileExistsError，pipeline 按 stop 策略退出。这次是评分目录续跑兼容缺口，不是新的模型 OOM。

`run_ui14_eval.py` 改为每次评分分配带 UUID 的独立目录，并将 `evaluation_run_dir` 写入评测状态及成功历史。旧固定目录和已有尝试不删除、不改名、不覆盖；预测缓存目录、模型及生成参数保持不变。完整 14 项及 Excel 仍直接复用，缺项重试仍按 step 更新；失败状态不会当作成功轮次。

新增 CPU 回归在 step 0、1000 分别实际调用仓库的旧 UI5 评分脚本，使用每项一条固定预测的 14 任务构造数据，GPU 推理和真实数据清单校验用 fixture 替代。覆盖旧固定目录已存在、Excel 缺行、状态缺任务、UI5 评分成功后 UI9 中断再恢复；验证每次评分路径独立，旧报告、预测及 checkpoint 配置的字节/mtime 不变，失败时已成功的 Excel 和历史不被覆盖，完成重试后 14 项和旧五类 best 正确。原始完整评测测试以前模拟了评分子进程，因此未捕获该外部评分器的目录限制；本次新回归直接运行评分子进程。

本地定向 9 项 CPU 评测回归通过（42.712 秒）；随后完整 104 项 CPU 回归全部通过（53.930 秒），覆盖推理调度与预测续跑、初始及周期评测、Excel、准备/提交链路、训练 pipeline 和采样缓存。未连接集群，未执行真实 GPU 推理或正式提交。恢复时只更新代码并使用原 submit 入口，保留当前运行目录和 checkpoint-1000，不归档、不清空、不重建数据缓存。
