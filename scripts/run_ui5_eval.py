...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--evaluation-split", choices=("test",), default="test")
    parser.add_argument(
...
def build_inference_command(
    ...
    *,
    ...
    if detector_crop_manifest is not None:
        command.extend(["--detector-crop-manifest", str(detector_crop_manifest)])
    ...
    if existing_manifest.is_file():
        ...
        try:
            ...
            previous_crop = previous.get("inference_crop", {})
...
def main() -> int:
    ...
    if args.require_cache_scope != "full_test":
        raise ValueError(
            "crop-only formal evaluation requires full_test cache scope"
        )
    ...
    try:
        ...
        # The shared command builder uses these resolved, digest-bound values for
        # both the normal pass and M32's optional soft-vs-observe raw ablation.
        args.parallel_inference_script = (
            args.project_root / "scripts" / "run_ui5_parallel_inference.py"
        )
        args.detector_crop_manifest = detector_crop_manifest
        args.detector_crop_manifest_digest = detector_crop_manifest_digest
        ...
        if (
            args.inference_crop_mode == "detector_scan"
            and "--save-raw-answer" not in current_command
        ):
        ...
        metadata.update(
            {
                ...
                "gated_metrics_json": (
                    str(gated_metrics_json)
                    if gated_metrics_json is not None
                    else None
                ),
            }
        )
