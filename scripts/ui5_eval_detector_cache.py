...
def validate_eval_detector_cache(
    ...
    *,
    ...
    if required_cache_scope in {"validation", "full_test", "external_test"} and int(expected_unique_images) <= 0:
        raise ValueError(
    ...
    if cache_scope not in {"preview", "validation", "full_test", "external_test"}:
    ...
    if cache_scope in {"validation", "full_test", "external_test"} and max_images_per_task != 0:
