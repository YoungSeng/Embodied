...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--eval-data-split", choices=("test",), default="test")
    ...
    parser.add_argument(
        "--eval-detector-cache-mode", choices=("readonly",), default="readonly"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Expected content-unique evaluation images; full test defaults to 1555"
        ),
    )
    ...
    parser.add_argument(
        ...
        dest="validation_early_stop",
        action="store_false",
        default=False,
        help="Compatibility flag; formal UI5 training never uses validation early stop",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use-detection-crops",
        action="store_true",
        help="Train from an audited crop recipe (requires --crop-audit-dir)",
    )
    parser.add_argument("--crop-audit-dir", default=None)
    parser.add_argument(
        "--crop-train-mode",
        choices=("full_only", "crop_only"),
        default=None,
    )
...
def build_submission_environment(args: argparse.Namespace) -> dict[str, str]:
    ...
    explicit = {
        ...
        "EVAL_DATA_SPLIT": getattr(args, "eval_data_split", "test"),
...
def render_job(args: argparse.Namespace) -> tuple[str, dict[str, Any]]:
    ...
    if getattr(args, "profile", None):
        env_keys = (*env_keys, "UI_TRAIN_PROFILE", "UI14_DATA_ROOT", "UI_TASK_REGISTRY", "UI_EVAL_MANIFEST", "UI14_CHECK_REPORT", "UI_EVAL_ANSWER_GRAMMAR", "UI_NUM_TASKS", "META_PATH", "OUTPUT_DIR", "LOGGING_STEPS", "SAMPLE_LOG_INTERVAL", "LOCANY_CPT_MODE")
    if args.scorer_root:
