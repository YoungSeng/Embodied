...
class DetectorScanPipelineTest(unittest.TestCase):
    ...
    def test_full_test_cache_rejects_one_task_with_1554_rows_before_geometry(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            scan_name = "horizontal_scan_v5_raw_detector_edge_aligned"
            ...
            for task_name in common.TASKS:
                row_count = 1554 if task_name == "content_missing" else 1555
                ...
                task_records.append(
                    {
                        "task": task_name,
                        "path": str(task_file),
                        "sha256": preparer.sha256_file(task_file),
                    }
                )
            (scan_root / "eval_detector_cache_ready.json").write_text(
                json.dumps(
                    {
                        "schema_version": 5,
                        "scan_name": scan_name,
                        ...
                        # The content-unique union is correct, but one task is short.
                        "expected_unique_images": 1555,
                        "dataset": {
                            "content_unique_images": 1555,
            ...
            with self.assertRaisesRegex(
                RuntimeError,
                r"row count mismatch for content_missing: 1554 != 1555",
            ):
                validate_eval_detector_cache(
                    root,
                    scan_name=scan_name,
    ...
    def test_crop_stage_writes_visualization_gallery_and_statistics_without_gt(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            ready_marker = json.loads(
                (scan_root / "eval_detector_cache_ready.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertEqual(
                {
                    record["task"]: record.get("jsonl_rows")
                    for record in ready_marker["dataset"]["task_files"]
                },
                {task_name: 1 for task_name in common.TASKS},
            )
    ...
    def test_parallel_preflight_rejects_short_task_before_workers(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            for task_name in common.TASKS:
                row_count = 1554 if task_name == "content_missing" else 1555
                (input_dir / common.TASK_JSONL[task_name]).write_text(
                    "{}\n" * row_count,
                    encoding="utf-8",
                )
            inference_script = root / "must_not_run.py"
            inference_script.write_text(
                "raise AssertionError('worker must not start')\n",
                encoding="utf-8",
            )
            argv = [
                "run_ui5_parallel_inference.py",
                "--checkpoint",
                str(checkpoint),
                "--processor-path",
                str(processor),
            ...
            with mock.patch.object(sys, "argv", argv), mock.patch.object(
                ...
                with self.assertRaisesRegex(
                    RuntimeError,
                    r"before worker launch for content_missing: 1554 != 1555",
                ):
                    parallel.main()
            worker_run.assert_not_called()

    def test_parallel_uses_full_test_marker_count_before_workers(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            checkpoint = root / "checkpoint"
            processor = root / "processor"
            ...
            scan_name = "horizontal_scan_v5_raw_detector_edge_aligned"
            scan_root = cache_root / scan_name
            checkpoint.mkdir()
            processor.mkdir()
            input_dir.mkdir()
            scan_root.mkdir(parents=True)
            ...
            for task_name in common.TASKS:
                row_count = 1554 if task_name == "content_missing" else 1555
                task_file = input_dir / common.TASK_JSONL[task_name]
                task_file.write_text("{}\n" * row_count, encoding="utf-8")
                task_records.append(
                    {
                        "task": task_name,
            ...
            detector_manifest = scan_root / "detector_scan_crops.jsonl"
    ...
    def test_detector_scan_without_raw_sidecars_fails_before_workers(self) -> None:
        ...
        self.assertIn('"--expected-images-per-task"', source)
        source = (SCRIPTS / "run_ui5_parallel_inference.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("detector_scan requires --save-raw-answer", source)

    def test_raw_and_frozen_scorers_pin_bbox_iou_point_one(self) -> None:
        for script_name in ("run_ui5_eval.py", "score_ui5_frozen_gate.py"):
            source = (SCRIPTS / script_name).read_text(encoding="utf-8")
            self.assertIn('"--iou_thresh",\n        "0.1",', source)
    ...
    def test_runtime_defaults_to_detector_scan_and_exposes_detector_settings(self) -> None:
        ...
        self.assertTrue(config["PROJECT_ROOT"].endswith("Embodied"))
        ...
        self.assertEqual(config["EVAL_DATA_SPLIT"], "test")
        ...
        self.assertTrue(config["EVAL_INPUT_DIR"].endswith("sicheng_workspace/data"))
        self.assertTrue(
            config["EVAL_DETECTOR_CACHE"].endswith(
                "work_dirs/ui5_eval_detector_cache_horizontal_v5"
            )
        )
